...
// 			for (var i = 0; i < nodes.length; i++) {
// 				var h = nodes[i];
// 				var h1 = h.innerText.split(" - ")[0];
// 				var tocNode = document.createElement("li");
// 				tocNode.innerHTML = "<a href=#" + h.id + ">" + h1 + "</a>";
// 				tocNode.className = "rapid-toc-node level-" + levelNum;
// 				var tocUl = tocElement.getElementsByTagName("UL")[0];
// 				tocUl.appendChild(tocNode);
// 				var nextLevelNum = levelNum + 1;
...
// 	var htmlraw = body.innerHTML;
// 	var regEm = /(\*|_)(.*?)\1/;
// 	htmlraw = replaceAll(htmlraw, regEm, '<span class="rapid-markdown-em">$2</span>');
// 	body.innerHTML = htmlraw;
// }
...
// 	init: function() {
// 		var domContainer = this.dom["container"];
// 		var htmlraw = domContainer.innerHTML;
// 		var htmlparts = htmlraw.split("\n");
// 		var html = "";
...
// 		parsed = parsed.replace(regH1, '<h1>$2</h1>');
// 		parsed = parsed.replace(regH2, '<h2>$2</h2>');
// 		parsed = parsed.replace(regA, "<a href='$2'>$1</a>");
// 		parsed = parsed.replace(regStrong, '<span class="rapid-markdown-strong">$2</span>');
// 		parsed = parsed.replace(regEm, '<span class="rapid-markdown-em">$2</span>');
// 		parsed = parsed.replace(regBlockquote, '<blockquote class="rapid-markdown-blockquote">$2</blockquote>');
// 		parsed = parsed.replace(regInlineCode, '<span class="rapid-markdown-inlinecode">$1</span>');
// 		parsed = parsed.replace(regImg, "<img src='$2' alt='$1'>");
// 		parsed = parsed.replace(regOl, "<ol><li>$1</li></ol>");
// 		parsed = parsed.replace(regUl, "<ul><li>$1</li></ul>");
// 		parsed = parsed.replace(regHr, "<hr>");
// 		parsed = parsed.replace(regCodeBlock, "<div class='rapid-markdown-codeblock'>$1</div>");

// 		parsed = parsed.replace(/\n{2}/g, '</p><p>');
// 		parsed = parsed.replace(/\n/g, '<br>');

// 		domContainer.innerHTML = parsed;
// 	}
// });

// TODO: namespace this global func
...
RAPID.element({
	...
	init: function() {
		...
		// TODO: if rapidOption contains a path, load the file and then create option buttons from the file contents
		var option = domContainer.dataset.rapidOption;
		if (option.includes(",")) {
			var labels = domContainer.dataset.rapidOption.split(",");
			RAPID.putOptionsToRow(labels, row);
		}
		else if (option.includes(".")) {
		 	//console.log("loading " + option);
			RAPID.load(option, function(data) {
...
RAPID.putOptionsToRow = function(labels, row) {
...
RAPID.element({
	...
	init: function() {
		...
		// TODO: if rapidOption contains a path, load the file and then create option buttons from the file contents
		var layout = domContainer.dataset.rapidLayout;
		var datasource = domContainer.dataset.rapidDatasource;
		var fixedwidth = domContainer.dataset.rapidWidth;
		RAPID.load(datasource + "/!list.txt", function(data) {
			var labels = data.split('\n');
			RAPID.doLayout(layout, labels, datasource, domContainer);
		});
	}
});

RAPID.doLayout = function(layout, labels, datasource, container) {
	for (var ii = 0; ii < labels.length; ii++) {
		(function() {
			var label = labels[ii];
			var btn = document.createElement("DIV");
			btn.className = "rapid-optionlist-button";
			btn.style.width = layout + "px";
			btn.addEventListener("click", function() {
				RAPID.popupImage(datasource + "/" + label + ".png");
			});
			container.appendChild(btn);

			var newlabel = document.createElement("SPAN");
			newlabel.innerHTML = label;
			newlabel.className = "rapid-optionlist-button-label";
			if (ii == 0) {
				newlabel.className += " selected";
			}
	...
	clear.className = "clearfloats";
	clear.style = "clear:both";
	container.appendChild(clear);
}

RAPID.popupImage = function(src) {
	var div = document.createElement("DIV");
	...
	init: function() {
		...
		for (var i = 0; i < 4; i++) {
			...
			for (var ii = 0; ii < 4; ii++) {
				...
				// 	var thetd = this;
				// 	thetd.setAttribute("contenteditable", "true");
				// 	thetd.classList.add("edit");
				// 	setTimeout(function() {
				// 		thetd.focus();
				// 	}, 50);
				// 	// TODO: if a formula was in this, show it now instead of the calculated value
					
				// });
				// td.addEventListener("focusin", function() {
				// 	var thetd = this;
				// 	if (thetd.formula) {
				// 		thetd.innerHTML = thetd.formula;
				// 	}
				// });
				...
				// 	// TODO: if a formula is entered, store/hide the formula and show the new calculated value
				// 	var cellContents = thetd.innerHTML;
				// 	if (cellContents.startsWith("=")) {
				// 		// store/hide the formula
				// 		thetd.formula = cellContents;
				// 		// show the calculated value
				// 		var newVal = eval(cellContents.substring(1));
				// 		// TODO: search the cellContents and replace all cell references with cell values

				// 		thetd.innerHTML = newVal;
				// 	}
				// });
