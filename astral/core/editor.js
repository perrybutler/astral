...
///////////////////////////////////////
	...
	function init() {
		...
		var connectButton = ctl("button buttonicon connect", null, "connect to server", null, toolsSection, "", function() {
			...
			if (ASTRAL.bridge) {
				...
				ASTRAL.error("bridge module not loaded. To use it, add it to 'modules' section in game.json.", 3000);
			}
		});
		...
		if (ASTRAL.bridge) {
			// TODO: we have several hard couplings by calling ASTRAL.bridge here...probably should move
			...
			ASTRAL.bridge.on("close", function() {
				// document.querySelectorAll(".connectionRequired").forEach(function(el) {
				// 	el.display = "none";
				// });
			...
			// subscribe to bridge dirlist which we use to update the PROJECT panel
			ASTRAL.bridge.on("*dirlist", function(payload) {
				console.log("POPULATE");
				populateProjectPanel(payload);
			});

			ASTRAL.bridge.on("aftersend", function(payload) {
				var info = ASTRAL.bridge.getNetcodeInfo();
			...
			ASTRAL.bridge.on("afterreceive", function(payload) {
				var info = ASTRAL.bridge.getNetcodeInfo();
				if (netcodestuff.innerHTML != info) {
					netcodestuff.innerHTML = info;
					flashDomElement(netcodestuff);
			...
			ASTRAL.bridge.on("*createfileresult", function(payload) {
			...
			ASTRAL.bridge.on("*renamefileresult", function(payload) {
				// var fi = ASTRAL.getFileInfo(payload[0]);
			...
			ASTRAL.bridge.on("*deletefileresult", function(payload) {
				var fi = ASTRAL.getFileInfo(payload[0]);
				if (fi.name) {
					// file was deleted
		...
		// 	netcodestuff.innerHTML = ASTRAL.bridge.getNetcodeInfo();
		// }, 1000);
	...
	function createFile() {
		//ASTRAL.bridge.sendNow("*createfile," + currentPath);
...
///////////////////////////////////////
	...
	function deleteProjectNode(node) {
		...
		ASTRAL.bridge.sendNow("*deletefile," + path);
	}

	function copyFile(node) {
		var path = currentPath + "/" + node.innerHTML;
		ASTRAL.bridge.sendNow("*copyfile," + path);
		// TODO: instead of hard coupled to bridge here, we could implement an ASTRAL.copyFile
		//	method which just sends a message to any listeners, and we can setup bridge as a 
		//	listener in bridge.js e.g. ASTRAL.on("sendNow", function() {})
	}
	...
	function makePrefab(obj) {
		...
		ASTRAL.bridge.sendNow("*makeprefab," + obj.name + "," + data);
	}
	...
	function beginRenameProjectNode(node, isNew) {
		...
		function doneRenaming() {
			...
			if (isNew) {
				...
				ASTRAL.bridge.sendNow("*createfile," + newpath);
			}
			else {
				var newpath = currentPath + "/" + node.innerHTML;
				ASTRAL.bridge.sendNow("*renamefile," + oldpath + "," + newpath);
	...
	function openProjectFile(event, path) {
		...
		if (event.ctrlKey) {
			...
			ASTRAL.bridge.sendNow("*exec," + path);
		}
		else {
...
///////////////////////////////////////
	...
	function getDir(path) {
		...
		ASTRAL.bridge.sendNow("*getdir," + path);
		currentPath = path;
	}
